...
const pool = new Pool({
  host: process.env.PGHOST, port: process.env.PGPORT, database: process.env.PGDATABASE, user: process.env.PGUSER, password: process.env.PGPASSWORD,
...
app.get('/:client', async (req,res) => {
  ...
  try {
    ...
    html = html.replace(/Posto Studio/g, c.name);
    html = html.replace(/<title>Posto Studio<\/title>/,'<title>'+c.name+'</title>');
    html = html.replace(/#3d8ce0/g, c.color);
    html = html.replace(/#0e3270/g, c.color_light);
    res.send(html);
  } catch(err) { res.sendFile(path.join(__dirname,'app.html')); }
});

app.get('/api/clients', async (req,res) => {
  try { const r = await pool.query('SELECT * FROM clients ORDER BY created_at DESC'); res.json(r.rows); }
...
app.post('/api/clients', async (req,res) => {
  ...
  try {
    const r = await pool.query('INSERT INTO clients (name,slug,sector,color,color_light,email,phone,notes) VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *',[name,slug,sector,color||'#3d8ce0',color_light||'#0e3270',email,phone,notes]);
    res.json(r.rows[0]);
  } catch(err) { res.status(500).json({error:err.message}); }
});
...
app.get('/api/demandes', async (req,res) => {
  try { const r = await pool.query('SELECT * FROM demandes ORDER BY created_at DESC'); res.json(r.rows); }
  catch(err) { res.status(500).json({error:err.message}); }
});

app.put('/api/demandes/:id', async (req,res) => {
  try { await pool.query('UPDATE demandes SET status=$1 WHERE id=$2',[req.body.status,req.params.id]); res.json({success:true}); }
  catch(err) { res.status(500).json({error:err.message}); }
});
...
app.post('/send-email', async (req,res) => {
  ...
  try {
    await pool.query('INSERT INTO demandes (client_name,client_email,type,message,style,video_url) VALUES ($1,$2,$3,$4,$5,$6)',[name,email,type,message,style,videoUrl||null]);
    await emailjs.send('service_qjbwtf8','template_j6vpg4b',{
      name:name||'Inconnu',email:email||'',type:type||'Demande',
      message:(message||'')+(videoUrl?'\n\nLIEN VIDEO : '+videoUrl:'\n\nPas de video jointe.'),
      style:style||'N/A'
    },{publicKey:'Gg1Wl2WLrhqbfxyxO',privateKey:'EnL1sdzNyW0aSuGlZRVM8'});
    res.json({success:true});
  } catch(err) { res.status(500).json({error:err.message||JSON.stringify(err)}); }
});
...
app.post('/generate', async (req,res) => {
  ...
  try {
    const response = await fetch('https://api.anthropic.com/v1/messages',{method:'POST',headers:{'Content-Type':'application/json','x-api-key':process.env.ANTHROPIC_API_KEY,'anthropic-version':'2023-06-01'},body:JSON.stringify({model:'claude-sonnet-4-20250514',max_tokens:800,messages:[{role:'user',content:prompt}]})});
    const data = await response.json();
